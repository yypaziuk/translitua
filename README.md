...
- **`index.html`** (the web app) is original work by the repository author and is free to use, modify, and distribute.
- **`scripts/*.jsx`** are derivative works of [a3-tools](https://github.com/agentyzmin/a3-tools), which is published **without a license**. They are redistributed here with attribution to the original authors; reuse rights are governed by the upstream project, not by this repository.
...
- **`index.html`** (веб-застосунок) — оригінальна робота автора репозиторію, вільна для використання, зміни та поширення.
- **`scripts/*.jsx`** — похідні від [a3-tools](https://github.com/agentyzmin/a3-tools), яка опублікована **без ліцензії**. Розповсюджуються тут з атрибуцією до оригінальних авторів; права на повторне використання визначає вихідний проєкт, а не цей репозиторій.
